...
DEFAULT_BACKGROUNDS = [
    BackgroundSpec(id="studio", name="Soft studio", value="radial-gradient(circle,#374151,#030712)", tags=["neutral"]),
    BackgroundSpec(id="cyberpunk-city", name="Cyberpunk city", value="linear-gradient(135deg,#111827,#312e81 45%,#ec4899)", synced_style_id="cyberpunk", tags=["coding", "debug"]),
    BackgroundSpec(id="cozy-library", name="Cozy library", value="linear-gradient(135deg,#422006,#92400e 48%,#f59e0b)", synced_style_id="cozy", tags=["writing", "warm"]),
    BackgroundSpec(id="glitch-grid", name="Glitch grid", value="linear-gradient(90deg,#020617,#164e63 50%,#7f1d1d)", synced_style_id="glitch", tags=["debug", "system"]),
]
...
DEFAULT_STYLES = [
    ...
    VisualStyle(
        ...
        voice=VoiceStyleSpec(pace=0.5, warmth=0.5, intensity=0.48, tags=["precise", "focused"]),
    ...
    VisualStyle(
        ...
        voice=VoiceStyleSpec(pace=0.38, warmth=0.82, intensity=0.28, tags=["warm", "gentle"]),
    ...
    VisualStyle(
        ...
        voice=VoiceStyleSpec(pace=0.56, warmth=0.42, intensity=0.62, tags=["diagnostic"]),
...
DEFAULT_WORKFLOW_RULES = [
    WorkflowStyleRule(workflow="coding", style_id="cyberpunk", background_id="cyberpunk-city", description="Use neon technical styling for coding tasks."),
    WorkflowStyleRule(workflow="book_writing", style_id="cozy", background_id="cozy-library", description="Use cozy styling for book-writing sessions."),
    WorkflowStyleRule(workflow="debugging", style_id="glitch", background_id="glitch-grid", description="Use glitch styling while editing or debugging the avatar system."),
]
...
def build_asset_index(root: str | Path, character_id: str | None = None) -> CharacterIndex:
    ...
    default_style_id = profile.get("default_style_id") or (styles[0].id if styles else "neutral")
    default_background_id = profile.get("default_background_id")
    if default_background_id is None:
        default_style = next((style for style in styles if style.id == default_style_id), None)
        default_background_id = default_style.default_background_id if default_style else (backgrounds[0].id if backgrounds else None)
